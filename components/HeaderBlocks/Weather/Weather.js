...
export const Weather =( {hide} ) => {
	...
	return (
		<>
			<Link href={`/weather`}>
				<button className={`${style.blockHeader} ${hide ? style.active : ''}`}>
					...
					<div className={style.contenBlcok}>
						{weather !== null ? (weather >= 0 ? `+${Math.round(weather)}°` : `${Math.round(weather)}°`) : ( width > 900 ? 'Загрузка...' : '...')}
					</div>
				</button>
			</Link>
		</>
